...
// that differ between the upstream QuantiX Drive product and any downstream
// branded forks. All fields are populated from environment variables at
// startup and are read-only thereafter.
type ProductConfig struct {
	// Name is the human-facing product name (e.g. "QuantiX Drive").
	Name string
	// Slug is the kebab-case identifier used in URLs, filenames, and logs
	// (e.g. "quantix-drive"). Must not contain slashes.
	Slug string
	...
	// (e.g. "qx_ak"). Legacy prefixes are still accepted for authentication;
	// only issuance uses this value.
	AgentKeyPrefix string
	// LegacyAgentKeyPrefixes lists historical prefixes that existing
	// production keys may start with. Downstream branded forks can set
	// AGENT_KEY_LEGACY_PREFIXES to preserve compatibility with previously
	// issued keys.
	LegacyAgentKeyPrefixes []string
	// PublicBaseURL is the canonical public URL for this deployment
	// (e.g. "https://app.quantixdrive.io"). Used only for informational
...
func (p *ProductConfig) IsAgentAPIKey(credential string) bool {
	...
	for _, legacy := range p.LegacyAgentKeyPrefixes {
