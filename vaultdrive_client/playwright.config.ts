...
export default defineConfig({
  ...
  webServer: {
    command:
      "cd .. && mkdir -p \"$UPLOAD_DIR\" && psql \"$E2E_ADMIN_DB_URL\" -tAc \"SELECT 1 FROM pg_database WHERE datname = '$E2E_DB_NAME'\" | grep -q 1 || psql \"$E2E_ADMIN_DB_URL\" -c \"CREATE DATABASE \\\"$E2E_DB_NAME\\\"\" && go run github.com/pressly/goose/v3/cmd/goose@latest -dir sql/schema postgres \"$DB_URL\" up && PORT=8090 go run .",
    env: e2eBackendEnv,
    url: baseURL,
