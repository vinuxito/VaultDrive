...
export function FileSecurityTimeline({ fileId }: { fileId: string }) {
  ...
  return <section className="brand-trust-shell rounded-2xl p-4 space-y-3">
    <h3 className="flex items-center gap-2 font-semibold text-foreground"><Clock3 className="h-4 w-4 shrink-0" />{t('coherence.trust.history', { defaultValue: 'Access activity snapshot' })}</h3>
    {loading ? <p role="status">{t('coherence.trust.historyLoading', { defaultValue: 'Loading recorded history…' })}</p> : error ?
      ...
      <p className="text-muted-foreground">{t('coherence.trust.historyEmpty', { defaultValue: 'No recorded events were returned. Check access settings to review current permissions.' })}</p> : <>
        <p className="text-xs text-muted-foreground">{t('coherence.trust.historyCount', { defaultValue: '{{count}} entries reconstructed from current records; past changes may be absent', count: events.length })}</p>
        <ol className="max-h-72 overflow-y-auto space-y-2">{events.map(event => <li key={event.id} className="brand-trust-panel rounded-xl p-3 space-y-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground break-words">{event.event_type === 'revoked' ? t('coherence.trust.closureTimeUnknown', { defaultValue: 'Closure time is unavailable in this snapshot.' }) : <><time dateTime={event.at}>{formatDate(event.at, i18n.language)}</time> · {relativeTime(event.at, i18n.language)}</>}</p>
        </li>)}</ol>
      </>}
  </section>;
}
