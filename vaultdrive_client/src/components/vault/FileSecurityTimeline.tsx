...
function toneIconBg(tone: string): string {
  if (tone === "warn") return "bg-amber-500/20 text-amber-700 dark:text-amber-300 ring-1 ring-amber-500/30";
  if (tone === "good") return "bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-500/30";
    ...
    <div className="brand-trust-shell rounded-[1.75rem] px-4 py-4 space-y-3">
      <div className="relative z-10 flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 text-muted-foreground">
          <Clock3 className="w-3.5 h-3.5 text-amber-700 dark:text-amber-200" />
      ...
      {loading ? (
        ...
        <div className="relative z-10 brand-trust-panel rounded-2xl flex items-center gap-3 px-4 py-4">
          <div className="w-6 h-6 rounded-full bg-amber-500/20 ring-1 ring-amber-400/20 flex items-center justify-center shrink-0">
            <Clock3 className="w-3 h-3 text-amber-700 dark:text-amber-300" />
          </div>
          <div>
            <p className="text-sm text-foreground">Security history is temporarily unavailable.</p>
            <p className="text-xs text-muted-foreground mt-0.5">The file remains protected; only the event feed could not be loaded.</p>
          </div>
        </div>
      ) : events.length === 0 ? (
        <div className="relative z-10 brand-trust-panel-strong rounded-2xl flex items-center gap-3 px-4 py-4">
          ...
          <div>
            <p className="text-sm text-foreground">No external access has occurred yet.</p>
            <p className="text-xs text-muted-foreground mt-0.5">Only you currently hold access to this file.</p>
          </div>
        </div>
      ) : (
          ...
          {events.map((event, idx) => (
            <div key={event.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                ...
                {idx < events.length - 1 && (
                  <span className="mt-1.5 w-px h-6 bg-border shrink-0" />
                )}
              </div>
              <div
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
                      <span className="inline-flex items-center rounded-full border border-border bg-background px-2 py-0.5 font-medium text-foreground">
                        {eventGroupLabel(event.event_type)}
                      </span>
                      <span className="inline-flex items-center rounded-full border border-border bg-background px-2 py-0.5 text-muted-foreground">
                        {toneLabel(event.tone)}
                      </span>
                    </div>
                    <p className="text-sm text-foreground leading-relaxed">{event.label}</p>
                  </div>
                  <span className="rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-medium text-foreground whitespace-nowrap">
