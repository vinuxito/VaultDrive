...
export function UpdateFolderShareLinkModal({
  ...
  async function handleSync(link: SyncableFolderShareLink) {
    ...
    if (!token || !credential) {
      setErrorMsg(
        "Open your vault with your current credential first, then try again.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        !link.owner_wrapped_folder_key &&
        (legacyUrls[link.id]?.trim() || undefined)
      ) {
        setLinks((prev) =>
          prev.map((entry) =>
            entry.id === link.id
              ? { ...entry, owner_wrapped_folder_key: "stored" }
              : entry,
          ),
        );
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to update the shared link.";
      setResults((prev) => ({ ...prev, [link.id]: message }));
    } finally {
      setSyncingId(null);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <Card
        className={cn(
          "w-full max-w-2xl border max-h-[calc(100dvh-2rem)] overflow-y-auto",
      ...
      >
        <CardHeader className={cn("border-b", "border-border")}>
          <div className="flex items-center justify-between">
            ...
            >
              <RefreshCw className={cn("w-5 h-5", "text-primary")} />
              Update Shared Link
            </CardTitle>
            <button
              type="button"
          ...
          >
            Use this to add newly uploaded files into an existing folder share.
            Newer links update directly. Older links need the original share URL
            once so the browser can recover the folder key.
          </div>
        </CardHeader>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {visibleLinks.map((link) => {
                ...
                const canSync =
                  !needsLegacyUrl || Boolean(legacyUrls[link.id]?.trim());
                return (
                  <div
                    key={link.id}
                    className={cn(
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1 min-w-0">
                        ...
                        <div className={cn("text-xs", "text-muted-foreground")}>
                          {needsLegacyUrl
                            ? "Legacy link, needs original URL"
                            : "Owner-recoverable link"}
                        </div>
                        {link.expires_at && (
                          <div
                      ...
                      <div
                        className={cn(
                          ...
                          result.includes("Failed") ||
                            result.includes("Paste") ||
                            result.includes("different")
                            ? "bg-destructive/10 text-destructive border-destructive/20"
                            : "bg-emerald-500/10 text-emerald-900 dark:text-emerald-100 border-emerald-500/20",
                        )}
                      >
                        <CheckCircle2
                          className={cn(
