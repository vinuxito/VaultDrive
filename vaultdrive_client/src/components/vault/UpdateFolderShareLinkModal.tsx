    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <Card className="w-full max-w-2xl mx-4 bg-gradient-to-br from-[#7d4f50] to-[#6b4345] border-white/10 text-white">
        ...
        <CardContent className="space-y-4 py-4 max-h-[70vh] overflow-y-auto">
          ...
          {loading ? (
            <div className="py-8 flex flex-col items-center gap-3 text-white/80">
              <Loader2 className="w-6 h-6 animate-spin" />
              <span>Loading shared links…</span>
            </div>
          ) : visibleLinks.length === 0 ? (
            <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-6 text-sm text-white/75">
            ...
            <div className="space-y-3">
              {visibleLinks.map((link) => {
                ...
                return (
                  <div key={link.id} className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1 min-w-0">
                        ...
                        {link.expires_at && (
                          <div className="text-xs text-white/60">Expires {new Date(link.expires_at).toLocaleString()}</div>
                        )}
                      </div>
                      <Button
                        type="button"
                    ...
                    {needsLegacyUrl && (
                      <div className="space-y-2">
                        <label htmlFor={`legacy-share-url-${link.id}`} className="text-xs font-medium text-white/80 flex items-center gap-2">
                    ...
                    {result && (
                      <div className={`flex items-start gap-2 rounded-lg px-3 py-2 text-sm ${result.includes("Failed") || result.includes("Paste") || result.includes("different") ? "bg-red-500/10 text-white/85 border border-red-300/30" : "bg-emerald-500/10 text-white/85 border border-emerald-300/30"}`}>
