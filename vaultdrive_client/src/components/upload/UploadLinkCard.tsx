        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-[11px] font-medium ${getStatusColor(status.variant)}`}>
                    {status.label}
                  </span>
                </div>
            ...
            <div className="flex items-center gap-4 mt-3 text-sm text-muted-foreground flex-wrap">
              ...
              {!token.max_files && !token.expires_at && <span>No expiration or file limits</span>}
            </div>
          </div>

          <Button variant="ghost" size="sm" onClick={onExpand} className="ml-2 shrink-0">
          ...
          <div className="p-4">
            ...
            {token.files && token.files.length > 0 ? (
              <div className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground mt-2">
                  ...
                  {token.files.length > 0 && (
                    <>
                      {" "}
                      • Last upload: {formatDate(token.files[token.files.length - 1].created_at)}
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-xs mt-1">Share this link to let external users upload files</p>
              </div>
            )}

            <div className="flex gap-2 mt-4 pt-4 border-t">
              {!token.used && status.variant !== "destructive" && (
                <Button variant="outline" size="sm" onClick={onDeactivate} className="flex-1 gap-1">
                  <X className="w-4 h-4" />
                  Seal route
                </Button>
              )}
              <Button variant="destructive" size="sm" onClick={onDelete} className="flex-1 gap-1">
