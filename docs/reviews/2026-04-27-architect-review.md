...
3. **Naming legacy.** 50 files in `docs/` still reference `abrn` / `QuantiX Drive`. The product was renamed but the docs weren't fully migrated. New contributors will be confused. A purge sweep + a short "this used to be called QuantiX Drive" note in CONTRIBUTING is a one-afternoon job.
